...
@section('title', 'Editar Administrador de Tenant')

@section('content')
    <div class="max-w-2xl mx-auto">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">Editar Administrador de Tenant</h1>

        <div class="bg-white rounded-lg shadow p-6">
            <form method="POST" action="{{ route('central.tenant-admins.update', $tenantAdmin) }}">
                @csrf
                @method('PUT')

                <div class="mb-4">
                    <label for="tenant_id" class="block text-gray-700 font-bold mb-2">Tienda/Tenant Asignado</label>
                    <select name="tenant_id" id="tenant_id" required
                        ...
                        @foreach($tenants as $tenant)
                            <option value="{{ $tenant->id }}" {{ old('tenant_id', $tenantAdmin->tenant_id) == $tenant->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $tenantAdmin->name) }}" required
                ...
                <div class="mb-4">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $tenantAdmin->email) }}" required
